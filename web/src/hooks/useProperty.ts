...
export function useUpdatePropertyComponents() {
  ...
  return useMutation({
    ...
    onSuccess: (updatedItem) => {
      ...
      queryClient.setQueryData<Property[]>(
        propertyKeys.lists(),
        (old) => old?.map(property => property.id === updatedItem.id ? updatedItem : property) || []
      );
    },
    onError: (error: any) => {
      toast({
...
export function useVerifyProperty() {
  ...
  return useMutation({
    mutationFn: ({ id, verificationType }: { id: string; verificationType: string }) =>
      verifyProperty(id, verificationType),
    onSuccess: (_, variables) => {
      ...
      toast({
        ...
        description: 'Property verification has been recorded in the ledger',
      });
    },
    onError: (error: any, variables) => {
      // Queue for offline sync if network error
