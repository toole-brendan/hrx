...
const ComponentList: React.FC<ComponentListProps> = ({
  ...
  const handleSelectChange = (
    ...
    if (isEditing && editingComponentData) {
      setEditingComponentData({
        ...editingComponentData,
        [field]: value as 'present' | 'missing' | 'damaged'
      });
    } else if (!isEditing) {
      setNewComponentData({
        ...newComponentData,
          ...
          {components.map((component) => (
            editingComponentId === component.id ? (
              <TableRow key={`edit-${component.id}`}>
                <TableCell>
                  <Input
                    value={editingComponentData?.name}
                    onChange={(e) => handleInputChange(e, 'name', true)}
                  />
                </TableCell>
                <TableCell>
                  <Input
                    value={editingComponentData?.nsn || ''}
                    onChange={(e) => handleInputChange(e, 'nsn', true)}
                  />
                </TableCell>
                <TableCell>
                  <Input
                    value={editingComponentData?.serialNumber || ''}
                    onChange={(e) => handleInputChange(e, 'serialNumber', true)}
                  />
                </TableCell>
                <TableCell>
                  <Input
                    type="number"
                    value={editingComponentData?.quantity}
                ...
                <TableCell className="text-center">
                  <Checkbox
                    checked={editingComponentData?.required}
                    onCheckedChange={(checked) => handleCheckboxChange(Boolean(checked), 'required', true)}
                  />
                </TableCell>
                <TableCell>
                  <Select
                    value={editingComponentData?.status}
                    onValueChange={(value) => handleSelectChange(value, 'status', true)}
                  >
                ...
                <TableCell>
                  <Input
                    value={editingComponentData?.notes || ''}
                    onChange={(e) => handleInputChange(e, 'notes', true)}
                  />
                </TableCell>
                <TableCell className="text-right space-x-1">
                  <Button
                    variant="ghost"
                    size="icon"
                    onClick={handleSaveEditingComponent}
                    title="Save Changes"
                  >
              ...
              <TableRow key={component.id}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {component.status !== 'present' && (
                      <Button
                        ...
                        size="icon"
                        className="h-6 w-6"
                        onClick={() => handleSimulateScanVerify(component)}
                        title="Simulate Scan/Verify"
                      >
                ...
                <TableCell className="text-right space-x-1">
                  <Button
                    variant="ghost"
                    size="icon"
                    onClick={() => handleStartEditing(component)}
          ...
          {isAdding && (
            <TableRow key="add-new">
              ...
              <TableCell>
                <Input
                  placeholder="NSN (Opt.)"
                  value={newComponentData.nsn || ''}
                  onChange={(e) => handleInputChange(e, 'nsn', false)}
                />
              </TableCell>
              <TableCell>
                <Input
                  placeholder="Serial (Opt.)"
                  value={newComponentData.serialNumber || ''}
                  onChange={(e) => handleInputChange(e, 'serialNumber', false)}
                />
              </TableCell>
              <TableCell>
                <Input
                  type="number"
                  value={newComponentData.quantity}
              ...
              <TableCell className="text-center">
                <Checkbox
                  checked={newComponentData.required}
                  onCheckedChange={(checked) => handleCheckboxChange(Boolean(checked), 'required', false)}
                />
              </TableCell>
              <TableCell>
                <Select
                  value={newComponentData.status}
                  onValueChange={(value) => handleSelectChange(value, 'status', false)}
                >
                  <SelectTrigger className="w-[100px]">
              ...
              <TableCell>
                <Input
                  ...
                  value={newComponentData.notes || ''}
                  onChange={(e) => handleInputChange(e, 'notes', false)}
                />
              </TableCell>
              <TableCell className="text-right space-x-1">
                <Button
