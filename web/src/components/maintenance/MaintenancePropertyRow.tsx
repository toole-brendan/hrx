    ...
    <div className="py-4 px-6 hover:bg-muted/30 transition-colors">
      <div className="grid grid-cols-[2fr_1fr_1fr_1fr_180px] gap-4 items-center">
        ...
        <div className="flex flex-col min-w-0">
          <h4 className="font-medium truncate" title={item.itemName}>
            {item.itemName}
          </h4>
          <div className="flex items-center text-xs text-muted-foreground mt-0.5">
            <Tag className="h-3 w-3 mr-1.5 inline flex-shrink-0" />
            <span className="truncate" title={item.serialNumber}>
              {item.serialNumber}
            </span>
          </div>
          <p className="text-xs mt-1.5 text-muted-foreground line-clamp-1">
            {item.description}
          </p>
        </div>
        ...
        <div className="flex flex-col text-xs">
          ...
          <div className="flex items-center mt-1.5">
            ...
            <span className="truncate" title={item.assignedTo || 'Unassigned'}>
