...
interface CalibrationItem {
  ...
  name: string;
  serialNumber: string;
  status: 'upcoming' | 'overdue' | 'due' | 'completed';
  dueDate: string;
...
const CalibrationManager: React.FC = () => {
  ...
  const mockData: CalibrationItem[] = [
    {
      id: 'cal1',
      name: 'AN/PRC-152 Radio',
      serialNumber: 'R2D2C3PO1',
      status: 'upcoming',
      dueDate: '2023-07-15',
    ...
    },
    {
      id: 'cal2',
      name: 'M150 RCO ACOG',
      serialNumber: 'AC90122',
      status: 'overdue',
      dueDate: '2023-04-30',
      lastCalibrated: '2022-10-30'
    },
    {
      id: 'cal3',
      name: 'AN/PVS-14 NVGs',
      serialNumber: 'NVG34287',
      status: 'due',
      dueDate: '2023-05-05',
      lastCalibrated: '2023-02-05'
    },
    {
      id: 'cal4',
      name: 'Lensatic Compass',
      serialNumber: 'C298371',
      status: 'completed',
      dueDate: '2023-06-01',
      lastCalibrated: '2023-05-01'
    }
  ];
  ...
  const StatusBadge = ({ status }: { status: string }) => {
    switch (status) {
      case 'upcoming':
        return (
          <Badge className="uppercase bg-blue-100/70 text-blue-700 border border-blue-600 text-[10px] tracking-wider px-2 rounded-none">
            UPCOMING
          </Badge>
        );
      case 'overdue':
        return (
          <Badge className="uppercase bg-red-100/70 text-red-700 border border-red-600 text-[10px] tracking-wider px-2 rounded-none">
            OVERDUE
          </Badge>
        );
      case 'due':
        return (
          <Badge className="uppercase bg-amber-100/70 text-amber-700 border border-amber-600 text-[10px] tracking-wider px-2 rounded-none">
            DUE NOW
          </Badge>
        );
      case 'completed':
      ...
      default:
        return (
          <Badge className="uppercase bg-gray-100/70 text-gray-700 border border-gray-600 text-[10px] tracking-wider px-2 rounded-none">
            {status.toUpperCase()}
          </Badge>
        );
    }
  ...
  const handleLogCalibrationClick = (item: CalibrationItem) => {
      ...
      <CardContent>
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid grid-cols-4 w-full rounded-none h-10 mb-6">
            <TabsTrigger value="upcoming" className="text-xs uppercase tracking-wider rounded-none">
              Upcoming
            </TabsTrigger>
            <TabsTrigger value="overdue" className="text-xs uppercase tracking-wider rounded-none">
              Overdue
            </TabsTrigger>
            <TabsTrigger value="due" className="text-xs uppercase tracking-wider rounded-none">
              Due Now
            </TabsTrigger>
            <TabsTrigger value="all" className="text-xs uppercase tracking-wider rounded-none">
              All Equipment
            </TabsTrigger>
          </TabsList>
            ...
            <div className="rounded-md border p-3 flex items-start gap-3">
              ...
              <div className="text-sm space-y-1">
                ...
                <p className="text-muted-foreground">
                  This item will need to be calibrated again in 180 days.
                </p>
              </div>
            </div>
          </div>
