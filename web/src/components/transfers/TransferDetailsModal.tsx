...
const TransferDetailsModal: React.FC<TransferDetailsModalProps> = ({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-2xl bg-card rounded-none">
        {/* Added bg-card and rounded-none for consistency */}
        <DialogHeader>
          ...
          <div>
            ...
            <div className="space-y-2.5 text-sm">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-right">{format(parseISO(transfer.date), 'PPpp')}</span>
              ...
              {transfer.rejectionReason && (
                <div className="pt-2">
                  ...
                  <p className="text-sm bg-muted/50 p-2 rounded">{transfer.rejectionReason}</p>
                  {/* Styling update */}
                </div>
              )}
            </div>
