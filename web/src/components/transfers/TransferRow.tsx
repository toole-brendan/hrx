...
const TransferRow: React.FC<TransferRowProps> = ({
  ...
  return (
    <div className="grid grid-cols-[100px_1.5fr_1fr_1fr_120px_140px] gap-4 border-b px-4 py-4 hover:bg-muted/50 transition-colors text-sm items-center">
      ...
      {/* From */}
      <div className={`truncate ${isSender && activeView === 'history' ? 'font-semibold' : ''}`} title={transfer.from}>
        {transfer.from}
      </div>
      
      {/* To */}
      <div className={`truncate ${isRecipient && activeView === 'history' ? 'font-semibold' : ''}`} title={transfer.to}>
        {transfer.to}
      </div>
        ...
        {isPendingIncoming ? (
          <>
            <Button
              variant="outline"
              size="sm"
              className="h-7 px-2 bg-transparent border-green-600 text-green-600 hover:bg-green-50 uppercase text-[10px] tracking-wider rounded-none font-semibold"
