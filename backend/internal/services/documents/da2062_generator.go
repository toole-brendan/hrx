...
// calculateEmptyRows calculates the number of empty rows needed to fill the form
func (g *DA2062Generator) calculateEmptyRows(itemCount int) []int {
	const minRows = 15 // Minimum rows to show on form
	emptyRowCount := 0
	if itemCount < minRows {
		emptyRowCount = minRows - itemCount
	}
	
	emptyRows := make([]int, emptyRowCount)
...
// HasEmbeddedTemplate checks if the embedded template is available
func HasEmbeddedTemplate() bool {
	return da2062HTMLTemplate != ""
}
...
func (g *DA2062Generator) GenerateDA2062HTML(
	...
	data := templateData{
		...
		Today:        time.Now().Format("02 Jan 2006"), // Military date format
		UnitInfo:     unitInfo,
		FromUser:     fromUser,
		ToUser:       toUser,
		Properties:   rows,
		EmptyRows:    g.calculateEmptyRows(len(properties)),
	...
	// Use embedded template if available, otherwise use default
	templateContent := da2062HTMLTemplate
	if templateContent == "" {
		log.Printf("Warning: Embedded template not found, using default template")
		templateContent = defaultDA2062HTMLTemplate
	}

	// Parse and execute template
	t, err := template.New("da2062").Parse(templateContent)
	if err != nil {
